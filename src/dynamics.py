...
class Dynamics: 
    ...
    def da_grav_dp_vec(self, x_vec):
        p_vec = x_vec[:3]
        p_norm = np.linalg.norm(p_vec)
        return -self.earth.mu * (np.eye(CFG.n_p) / p_norm**3 - 3 * np.outer(p_vec, p_vec) / p_norm**5)

    def d2a_grav_dp_vec_dp_vecT(self, x_vec):
        p_vec = x_vec[:3]
        p_norm = np.linalg.norm(p_vec)
        term1_der = -3 / p_norm**5 * np.kron(np.eye(CFG.n_p), p_vec)
        term2_der = 1 / p_norm**5 * (np.kron(np.eye(CFG.n_p).reshape(-1, 1), p_vec.T) + np.kron(p_vec, np.eye(CFG.n_p))) - 5 / p_norm**7 * np.kron(p_vec, np.outer(p_vec, p_vec))
    ...
    def a_J2(self, x_vec):
        ...
        x, y, z = p_vec
        p_norm = np.linalg.norm(p_vec)
        return -3 * self.earth.J_2 * self.earth.mu * self.earth.R**2 / (2 * p_norm**5) * np.array([(1 - 5 * z**2 / p_norm**2) * x, 
                                                                                                   (1 - 5 * z**2 / p_norm**2) * y, 
                                                                                                   (3 - 5 * z**2 / p_norm**2) * z])
    ...
    def da_J2_dp_vec(self, x_vec):
        ...
        x, y, z = p_vec
        p_norm = np.linalg.norm(p_vec)
        result = -3 * self.earth.J_2 * self.earth.mu * self.earth.R**2 / (2 * p_norm**9) * np.array([[-4 * x**4 - 3 * x**2 * y**2 + 27 * x**2 * z**2 + y**4 - 3 * y**2 * z**2 - 4 * z**4, -5 * x**3 * y - 5 * x * y**3 + 30 * x * y * z**2, -15 * x**3 * z - 15 * x * y**2 * z + 20 * x * z**3],
    ...
    def d2a_J2_dp_vec_dp_vecT(self, x_vec):
        ...
        x, y, z = p_vec
        p_norm = np.linalg.norm(p_vec)
        result = -3 * self.earth.J_2 * self.earth.mu * self.earth.R**2 / (2 * p_norm**11) * np.array([[20*x**5 + 5*x**3*y**2 - 205*x**3*z**2 - 15*x*y**4 + 75*x*y**2*z**2 + 90*x*z**4, 30*x**4*y + 25*x**2*y**3 - 255*x**2*y*z**2 - 5*y**5 + 25*y**3*z**2 + 30*y*z**4, 90*x**4*z + 75*x**2*y**2*z - 205*x**2*z**3 - 15*y**4*z + 5*y**2*z**3 + 20*z**5],
    ...
    def a_drag(self, x_vec):
        ...
        return -1 / 2 * CFG.C_drag * CFG.A_drag / CFG.m * rho * v_rel_norm * v_vec_rel
    ...
    def da_drag_dp_vec(self, x_vec):
        ...
        dv_vec_rel_dp_vec = np.array([[0, self.earth.omega, 0], [-self.earth.omega, 0, 0], [0, 0, 0]])
        return -1 / 2 * CFG.C_drag * CFG.A_drag / CFG.m * (rho * (v_rel_norm * np.eye(3) + v_vec_rel * v_vec_rel.T / v_rel_norm) @ dv_vec_rel_dp_vec + v_rel_norm * v_vec_rel * drho_dp_vec.T)
    ...
    def da_drag_dv_vec(self, x_vec):
        ...
        v_vec_rel = v_vec - np.cross(omega_vec.reshape(3,), p_vec.reshape(3,)).reshape(3, 1)
        v_rel_norm = np.linalg.norm(v_vec_rel)
        return -1 / 2 * CFG.C_drag * CFG.A_drag / CFG.m * rho * (v_rel_norm * np.eye(3) + v_vec_rel * v_vec_rel.T / v_rel_norm)
    ...
    def d2a_drag_dp_vec_dp_vecT(self, x_vec):
        ...
        d2rho_dp_vec2 = rho / H**2 * p_vec * p_vec.T / p_norm**2 - rho / H * (np.eye(3) / p_norm - p_vec * p_vec.T / p_norm**3)
        dv_vec_rel_dp_vec = np.array([[0, self.earth.omega, 0], [-self.earth.omega, 0, 0], [0, 0, 0]])
        ...
        term2 = term21 + term22
        return -1 / 2 * CFG.C_drag * CFG.A_drag / CFG.m * (term1 + term2)
    ...
    def d2a_drag_dv_vec_dv_vecT(self, x_vec):
        ...
        v_vec_rel = v_vec - np.cross(omega_vec.reshape(3,), p_vec.reshape(3,)).reshape(3, 1)
        v_rel_norm = np.linalg.norm(v_vec_rel)
        return -1 / 2 * CFG.C_drag * CFG.A_drag / CFG.m * rho * (1 / v_rel_norm * (np.kron(np.eye(3), v_vec_rel) + np.kron(np.eye(3).reshape(-1, 1), v_vec_rel.T) + np.kron(v_vec_rel, np.eye(3))) - 1 / v_rel_norm**3 * np.kron(v_vec_rel, np.outer(v_vec_rel, v_vec_rel)))

    def diff_eq(self, x_vec):
        x_dot_vec = np.zeros_like(x_vec)
        for i in range(CFG.N):
            x_vec_i = x_vec[i * CFG.n_x : i * CFG.n_x + CFG.n_x]
            v_vec_i = x_vec_i[CFG.n_p : CFG.n_x] 
            x_dot_vec[i * CFG.n_x : i * CFG.n_x + CFG.n_x] = np.concatenate((v_vec_i, self.a_grav(x_vec_i) + self.a_J2(x_vec_i) + self.a_drag(x_vec_i)))
        return x_dot_vec

    def Ddiff_eq(self, x_vec):
        first_order_der = np.zeros((CFG.n, CFG.n))
        for i in range(CFG.N):
            x_vec_i = x_vec[i * CFG.n_x : i * CFG.n_x + CFG.n_x]
            first_order_der[i * CFG.n_x : i * CFG.n_x + CFG.n_p, i * CFG.n_x + CFG.n_p : i * CFG.n_x + CFG.n_x] = np.eye(CFG.n_p)
            first_order_der[i * CFG.n_x + CFG.n_p : i * CFG.n_x + CFG.n_x, i * CFG.n_x : i * CFG.n_x + CFG.n_p] = self.da_grav_dp_vec(x_vec_i) + self.da_J2_dp_vec(x_vec_i) + self.da_drag_dp_vec(x_vec_i)
            first_order_der[i * CFG.n_x + CFG.n_p : i * CFG.n_x + CFG.n_x, i * CFG.n_x + CFG.n_p : i * CFG.n_x + CFG.n_x] = self.da_drag_dv_vec(x_vec_i)
        return first_order_der

    def Hdiff_eq(self, x_vec):
        second_order_der = np.zeros((CFG.n, CFG.n, CFG.n))
        for i in range(CFG.N):
            x_vec_i = x_vec[i * CFG.n_x : i * CFG.n_x + CFG.n_x]
            aux_pp = (self.d2a_grav_dp_vec_dp_vecT(x_vec_i) + self.d2a_J2_dp_vec_dp_vecT(x_vec_i) + self.d2a_drag_dp_vec_dp_vecT(x_vec_i)).reshape((CFG.n_p, CFG.n_p, CFG.n_p))
            aux_pv = self.d2a_drag_dp_vec_dv_vecT(x_vec_i).reshape((CFG.n_p, CFG.n_p, CFG.n_p))
            aux_vp = self.d2a_drag_dv_vec_dp_vecT(x_vec_i).reshape((CFG.n_p, CFG.n_p, CFG.n_p))
            aux_vv = self.d2a_drag_dv_vec_dv_vecT(x_vec_i).reshape((CFG.n_p, CFG.n_p, CFG.n_p))
            for j in range(CFG.n_p): 
                second_order_der[i * CFG.n_x + CFG.n_p : i * CFG.n_x + CFG.n_x, i * CFG.n_x : i * CFG.n_x + CFG.n_p, i * CFG.n_x + j] = aux_pp[:, :, j]
                second_order_der[i * CFG.n_x + CFG.n_p : i * CFG.n_x + CFG.n_x, i * CFG.n_x : i * CFG.n_x + CFG.n_p, i * CFG.n_x + j + CFG.n_p] = aux_pv[:, :, j]
                second_order_der[i * CFG.n_x + CFG.n_p : i * CFG.n_x + CFG.n_x, i * CFG.n_x + CFG.n_p : i * CFG.n_x + CFG.n_x, i * CFG.n_x + j] = aux_vp[:, :, j]
                second_order_der[i * CFG.n_x + CFG.n_p : i * CFG.n_x + CFG.n_x, i * CFG.n_x + CFG.n_p : i * CFG.n_x + CFG.n_x, i * CFG.n_x + j + CFG.n_p] = aux_vv[:, :, j]
        return second_order_der.reshape((CFG.n * CFG.n, CFG.n))
    ...
    def Df(self, dt, x_old):
        ...
        Dk2 = self.Ddiff_eq(x_old + dt / 2 * k1) @ (np.eye(CFG.n) + dt / 2 * Dk1)
        Dk3 = self.Ddiff_eq(x_old + dt / 2 * k2) @ (np.eye(CFG.n) + dt / 2 * Dk2)
        Dk4 = self.Ddiff_eq(x_old + dt * k3) @ (np.eye(CFG.n) + dt * Dk3)
        return np.eye(CFG.n) + dt / 6 * (Dk1 + 2 * Dk2 + 2 * Dk3 + Dk4)
    ...
    def Hf(self, dt, x_old):
        ...
        Hk2 = np.kron(np.eye(CFG.n), np.eye(CFG.n) + dt / 2 * Dk1).T @ self.Hdiff_eq(x_old + dt / 2 * k1) @ (np.eye(CFG.n) + dt / 2 * Dk1) + np.kron(self.Ddiff_eq(x_old + dt / 2 * k1), np.eye(CFG.n)) @ (dt / 2 * Hk1)
        Hk3 = np.kron(np.eye(CFG.n), np.eye(CFG.n) + dt / 2 * Dk2).T @ self.Hdiff_eq(x_old + dt / 2 * k2) @ (np.eye(CFG.n) + dt / 2 * Dk2) + np.kron(self.Ddiff_eq(x_old + dt / 2 * k2), np.eye(CFG.n)) @ (dt / 2 * Hk2)
        Hk4 = np.kron(np.eye(CFG.n), np.eye(CFG.n) + dt * Dk3).T @ self.Hdiff_eq(x_old + dt * k3) @ (np.eye(CFG.n) + dt * Dk3) + np.kron(self.Ddiff_eq(x_old + dt * k3), np.eye(CFG.n)) @ (dt * Hk3)
