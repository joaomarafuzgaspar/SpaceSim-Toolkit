...
class Newton:
    def __init__(self):   
        ...
        # self.o = config.o
        # self.R = config.R
        self.o = config.o_tree
        self.R = config.R_tree
        self.dyn = Dynamics()
    ...
    def Dh(self, x_vec):
        ...
        for k, (i, j) in enumerate([(1, 0), (2, 0), (3, 0)], start=config.n_p):
            d = p_vecs[j] - p_vecs[i]
    ...
    def Hh(self, x_vec):
        ...
        def hessian_relative_range(d, norm_d):
            I = np.eye(config.n_p)
        ...
        for k, (i, j) in enumerate([(1, 0), (2, 0), (3, 0)], start=config.n_p):
            d = p_vecs[j] - p_vecs[i]
            norm_d = np.linalg.norm(d)
            hess_d = hessian_relative_range(d, norm_d)
    ...
    def HJ(self, k, Y, x_vec):
        ...
        for tau in range(k - self.H + 1, k + 1):
            ...
            DSTM = np.kron(np.eye(config.n), STM).T @ self.dyn.Hf(self.dt, x_vec) @ STM + np.kron(self.dyn.Df(self.dt, x_vec), np.eye(config.n)) @ DSTM
            STM = self.dyn.Df(self.dt, x_vec) @ STM
            x_vec = self.dyn.f(self.dt, x_vec)
        return HJ_x
    ...
    def solve_MHE_problem_with_metrics(self, k, Y, x_init, x_true_initial, x_true_end):
        ...
        for iteration in range(self.max_iterations + 1):
            ...
            J_x = self.J(k, Y, x)
            DJ_x = self.DJ(k, Y, x)
            HJ_x, HJ_x_eigenvalues, A_norm, B_norm, C_norm = self.HJ_with_metrics(k, Y, x)
