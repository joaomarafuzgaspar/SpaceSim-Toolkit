    ...
    def solve_MHE_problem(self, k, Y, x_init, x_true_initial, x_true_end):
        ...
        for iteration in range(self.max_iterations + 1):
            ...
            if (
                ...
                if iteration == 0:
                    print(
                        f"[Newton] Before applying the algorithm\nCost function: {cost_value}\nGradient norm: {gradient_norm_value}\nGlobal estimation error: {np.linalg.norm(x - x_true_initial)} ({np.linalg.norm(x - x_true_initial) / np.linalg.norm(x_true_initial) * 100}%)"
                    )
                else:
                    print(
                        f"[Newton] Iteration {iteration}\nCost function: {cost_value} ({cost_value_change:.2f}%)\nGradient norm: {gradient_norm_value} ({gradient_norm_value_change:.2f}%)\nGlobal estimation error: {np.linalg.norm(x - x_true_initial)} ({np.linalg.norm(x - x_true_initial) / np.linalg.norm(x_true_initial) * 100}%) ({global_estimation_error_change:.2f}%)"
                    )
            ...
            # Cold-start
            d_k = np.random.randn(len(c), 1)

            # Preconditioner
            P = np.zeros_like(HJ_x)
            ...
            threshold = 0.01  # %
            max_iter = 24
            err_rel = [100 * np.linalg.norm(d_k - d_sol) / np.linalg.norm(d_sol)]
            ...
            for num_iter in range(max_iter):
                ...
                # Update tracker
                rel_err = 100 * np.linalg.norm(d_k - d_sol) / np.linalg.norm(d_sol)
                err_rel.append(rel_err)

                # Check stopping criterion
                if rel_err < threshold:
                    print(
                        f"[PCG] Converged at iteration {num_iter} | Relative error: {rel_err}%"
                    )
                    break

                z = solve(P, r)  # tree pass
                rho = r.T @ z  # tree pass
                if num_iter > 0:
                    beta = rho / rho1
                    p = z + beta * p
                ...
                q = M @ p  # comm. with neighbors
                alpha = rho / (p.T @ q)  # tree pass
                d_k = d_k + alpha * p
                r = r - alpha * q
                ...
                print(
                    f"[PCG] Max iterations reached without convergence. | Relative error: {err_rel[-1]}%"
                )
