...
class SimulationConfig:
    name_of_file: str = "config_ts_60_H_100.py"
    ...
    H: int = 100  # Window size [timesteps]
    ...
    o_deputy: int = 1  # Deputy observation vector dimension
    o: int = (
    ...
    R_deputies: np.ndarray = (
        np.diag(np.concatenate([r_deputy_pos * np.ones(number_of_deputies * o_deputy)]))
        ** 2
    )
    ...
    p_pos_initial: float = 0  # [m]
    p_vel_initial: float = 0  # [m / s]
    ...
    def h(cls, x_vec):
        ...
        distances = [
            np.linalg.norm(p_vecs[i] - p_vecs[j]) for (i, j) in [(1, 0), (2, 0), (3, 0)]
        ]
