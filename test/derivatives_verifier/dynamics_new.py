...
sat = SatelliteDynamics()

def a_grav(x_vec):
    p_vec = x_vec[:3]
    p_norm = np.linalg.norm(p_vec)
    return -earth.mu * p_vec / p_norm**3

def da_grav_dp_vec(x_vec):
    p_vec = x_vec[:3]
    n = len(p_vec)
    p_norm = np.linalg.norm(p_vec)
    return -earth.mu * (np.eye(n) / p_norm**3 - 3 * np.outer(p_vec, p_vec) / p_norm**5)

def d2a_grav_dp_vec_dp_vecT(x_vec):
    p_vec = x_vec[:3]
    n = len(p_vec)
    p_norm = np.linalg.norm(p_vec)
    ...
    return -earth.mu * (term1_der - 3 * term2_der)

def a_J2(x_vec):
    p_vec = x_vec[:3]
    x, y, z = p_vec
    p_norm = np.linalg.norm(p_vec)
    return -3 * earth.J_2 * earth.mu * earth.R**2 / (2 * p_norm**5) * np.array([(1 - 5 * z**2 / p_norm**2) * x, 
                                                                                (1 - 5 * z**2 / p_norm**2) * y, 
                                                                                (3 - 5 * z**2 / p_norm**2) * z])

def da_J2_dp_vec(x_vec):
    p_vec = x_vec[:3]
    x, y, z = p_vec
    p_norm = np.linalg.norm(p_vec)
    result = -3 * earth.J_2 * earth.mu * earth.R**2 / (2 * p_norm**9) * np.array([[-4 * x**4 - 3 * x**2 * y**2 + 27 * x**2 * z**2 + y**4 - 3 * y**2 * z**2 - 4 * z**4, -5 * x**3 * y - 5 * x * y**3 + 30 * x * y * z**2, -15 * x**3 * z - 15 * x * y**2 * z + 20 * x * z**3],
...
def d2a_J2_dp_vec_dp_vecT(x_vec):
    p_vec = x_vec[:3]
    x, y, z = p_vec
    p_norm = np.linalg.norm(p_vec)
    result = -3 * earth.J_2 * earth.mu * earth.R**2 / (2 * p_norm**11) * np.array([[20*x**5 + 5*x**3*y**2 - 205*x**3*z**2 - 15*x*y**4 + 75*x*y**2*z**2 + 90*x*z**4, 30*x**4*y + 25*x**2*y**3 - 255*x**2*y*z**2 - 5*y**5 + 25*y**3*z**2 + 30*y*z**4, 90*x**4*z + 75*x**2*y**2*z - 205*x**2*z**3 - 15*y**4*z + 5*y**2*z**3 + 20*z**5],
...
def a_drag(x_vec):
    ...
    return -1 / 2 * sat.C_drag * sat.A_drag / sat.m * rho * v_rel_norm * v_vec_rel
...
def da_drag_dp_vec(x_vec):
    ...
    v_vec_rel = v_vec - np.cross(omega_vec.reshape(3,), p_vec.reshape(3,)).reshape(3, 1)
    v_rel_norm = np.linalg.norm(v_vec_rel)
    drho_dp_vec = -rho / atm.get_H(h) * p_vec / p_norm
    dv_vec_rel_dp_vec = np.array([[0, earth.omega, 0], [-earth.omega, 0, 0], [0, 0, 0]])
    return -1 / 2 * sat.C_drag * sat.A_drag / sat.m * (rho * (v_rel_norm * np.eye(3) + v_vec_rel * v_vec_rel.T / v_rel_norm) @ dv_vec_rel_dp_vec + v_rel_norm * v_vec_rel * drho_dp_vec.T)
...
def da_drag_dv_vec(x_vec):
    ...
    v_vec_rel = v_vec - np.cross(omega_vec.reshape(3,), p_vec.reshape(3,)).reshape(3, 1)
    v_rel_norm = np.linalg.norm(v_vec_rel)
    return -1 / 2 * sat.C_drag * sat.A_drag / sat.m * rho * (v_rel_norm * np.eye(3) + v_vec_rel * v_vec_rel.T / v_rel_norm)
...
def d2a_drag_dp_vec_dp_vecT(x_vec):
    ...
    term2 = term21 + term22
    return -1 / 2 * sat.C_drag * sat.A_drag / sat.m * (term1 + term2)
...
def d2a_drag_dv_vec_dv_vecT(x_vec):
    ...
    v_vec_rel = v_vec - np.cross(omega_vec.reshape(3,), p_vec.reshape(3,)).reshape(3, 1)
    v_rel_norm = np.linalg.norm(v_vec_rel)
    return -1 / 2 * sat.C_drag * sat.A_drag / sat.m * rho * (1 / v_rel_norm * (np.kron(np.eye(3), v_vec_rel) + np.kron(np.eye(3).reshape(-1, 1), v_vec_rel.T) + np.kron(v_vec_rel, np.eye(3))) - 1 / v_rel_norm**3 * np.kron(v_vec_rel, np.outer(v_vec_rel, v_vec_rel)))