...
class CentralizedNewton:
    ...
    def h(self, x_vec):
        p_vecs = [x_vec[i:i+3] for i in range(0, config.n, 6)]
        d = [p_vecs[i] - p_vecs[j] for (i, j) in [(1, 0), (2, 0), (3, 0)]]
        return np.concatenate((p_vecs[0], np.array(d).reshape(-1, 1)))
    ...
    def Dh(self, x_vec):
        ...
        for k, (i, j) in enumerate([(1, 0), (2, 0), (3, 0)], start=1):
            first_order_der[3 * k : 3 * (k + 1), i*6:i*6+3] = np.eye(3)
            first_order_der[3 * k : 3 * (k + 1), j*6:j*6+3] = -np.eye(3)
        
        return first_order_der

    def Hh(self, x_vec):
        return np.zeros((self.o, config.n, config.n)).reshape((self.o * config.n, config.n))
